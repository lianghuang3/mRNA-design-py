...
''' modified CKY for mRNA design
    problems:
    (1) overcounting grammar (cky vs. nussinov; see LD paper pseudocode
    (2) no "protein DFA"; (0,0) for codon-DFA node; should be (i, 0)           
'''
...
def cky(protein):
    ...
    def update(i, i_node, j, j_node, value, backpointer): # back is either k or (i_nuc, j_nuc)
        ...
        best[i, i_node, j, j_node] = np.logaddexp(best[i, i_node, j, j_node], value)
    ...
    best = defaultdict(lambda : -np.inf) # so that 0 might be answer
    back = {}
    print(protein)
                            ...
                            for jminus1_node, j_nuc in j_graph.left_edges[j_node]:
                                if i_nuc + j_nuc in match:
                                    update(i, i_node, j, j_node,
                                           best[i+1, iplus1_node, j-1, jminus1_node] + np.log(match[i_nuc + j_nuc]),
