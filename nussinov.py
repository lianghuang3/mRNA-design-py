...
class Float(float): # log-space
    ...
    __add__ = lambda self, other: Float(np.logaddexp(self, other))

    logzero = lambda : Float(-np.inf) # can't do logzero = Float.logzero() -- will get "Float" is undefined
...
if __name__ == "__main__":
    aa_graphs, _ = network.read_wheel("coding_wheel.txt") # TODO: replace OLD with NEW
    for line in sys.stdin:
        protein = line.split()
        graph = network.Lattice.protein_graph(protein, aa_graphs)
        print(nussinov(graph))
